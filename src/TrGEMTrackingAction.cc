...
void TrGEMTrackingAction::PreUserTrackingAction( const G4Track* aTrack )
{
  ...
  G4String partName = aTrack->GetParticleDefinition()->GetParticleName();
  G4String process;
  ...
  TrGEMAnalysis::GetInstance()->SaveGeneratingTrack(partCode, partName, process, energy, volume, trackID, parentID);
}
